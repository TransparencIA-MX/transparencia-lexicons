...
| `organizations` | string[] | | **Deprecated** — use `organizationEntities` instead |
...
| `relatedLegislation` | string[] | | **Deprecated** — use `structuredRefs` instead |
...
- **`organizations`** (string[]) is kept but deprecated — use `organizationEntities` for new records
- **`relatedLegislation`** (string[]) is kept but deprecated — use `structuredRefs` for new records
- **`politicalOrientation`** moved from required to optional — non-political articles no longer need a forced value
- **No existing fields were removed** — all v1 records remain valid v2 records
...
> **Note:** The GraphQL field names above reflect the v2 schema. If you have existing queries using old field names (e.g., `organizations`, `relatedLegislation`) or old Spanish `knownValues` (e.g., `izquierda`), they will continue to work — see [Backward Compatibility](#backward-compatibility).
